...
for i in sentence1:
    for j in sentence2:
        ...
        ground_truth2.append(j[0] + ' do not' + j[1:])
    ...
    try:
        ...
        if cosine_scores[0][0] < 0.5 and cosine_scores2[0][0] < 0.5 and m < 7:
            try:
                ...
                # correct = 'The conclusion is wrong. Given \"If p, then q\" and \"¬q\" are true, where p and q represent statements, we can infer that ¬p is true, so the conclusion is that ' + ground_truth[i] + '\n' + 'You should use this guideline to answer the following questions.'
