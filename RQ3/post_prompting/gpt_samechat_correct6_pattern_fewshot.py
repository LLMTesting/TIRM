...
while i < len(pattern6_list):
    ...
    if i % 10 == 0:
        ...
        content = ''
    if m < 6:
        content += str(m) + '. Question: ' + pattern6_list[i] + '\nExplanation: Given \"p or q\" and \"p\" are true, where p and q represent statements, we can not infer whether q is true, so there is no conclusion. \nAnswer: there is no conclusion.\n'
        m += 1
        i += 1
        continue
    if m == 6:
    ...
    if m >= 7:
        message = {"role": "user", "content": pattern6_list[i]}
    ...
    try:
        ...
        pattern1_dict['question'] = pattern6_list[i]
