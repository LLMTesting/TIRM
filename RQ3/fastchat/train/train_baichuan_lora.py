...
def apply_prompt_template(sources, systems=None):
    conv = get_conversation_template("baichuan2-chat")
...
def mask_targets(conversations, targets, tokenizer, conv):
    ...
    for conversation, target in zip(conversations, targets):
        ...
        turns = conversation.split("<reserved_106>")
        cur_len = 0
        target[:cur_len] = IGNORE_TOKEN_ID
        for i, turn in enumerate(turns):
            if turn == "":
                continue
            turn = "<reserved_106>" + turn
    ...
    if len(sources) <= 10000:
        ...
        with Pool() as p:
            conversations, conv = p.apply_async(
                apply_prompt_template, (sources, systems) # apply_prompt_template, (sources, tokenizer, systems)
            ).get()
...
def make_supervised_data_module(
    ...
    if data_path.endswith(".json"):
        train_raw_data = json.load(open(data_path, "r"))
    elif data_path.endswith(".jsonl"):
        with jsonlines.open(data_path, mode="r") as reader:
            train_raw_data = [item for item in reader]
    train_dataset = dataset_cls(train_raw_data, tokenizer=tokenizer)

    if data_args.eval_data_path:
        eval_data_path = data_args.eval_data_path
        if eval_data_path.endswith(".json"):
            eval_raw_data = json.load(open(eval_data_path, "r"))
        elif eval_data_path.endswith(".jsonl"):
            with jsonlines.open(eval_data_path, mode="r") as reader:
                eval_raw_data = [item for item in reader]
