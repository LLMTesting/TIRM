...
def preprocess_llama2(
    ...
    roles = {"human": conv.roles[0], "gpt": conv.roles[1], "system": "system"}
    ...
    for i, source in enumerate(sources):
        ...
        for j, sentence in enumerate(source):
            ...
            if role != conv.roles[j % 2]:
                bad_msg = True
                break
            assert role == conv.roles[j % 2], f"{role}:{i}"
            conv.append_message(role, sentence["value"])
        if not bad_msg:
            conversations.append(conv.get_prompt())
...
# preprocess_openbuddy
def preprocess_openbuddy(
    sources,
    tokenizer: transformers.PreTrainedTokenizer,
) -> Dict:
    conv = get_conversation_template("openbuddy")
    conv.system_message = ""
    roles = {"human": conv.roles[0], "gpt": conv.roles[1], "system": "system"}
    ...
    for conversation, target in zip(conversations, targets):
        ...
        target[cur_len] = tokenizer.eos_token_id
        target[cur_len + 1:] = IGNORE_TOKEN_ID
...
preprocess = preprocess_openbuddy
