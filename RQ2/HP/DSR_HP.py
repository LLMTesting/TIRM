...
while i < len(pattern5_list):
    ...
    if i % 10 == 0:
        ...
        content = ''
    if m < 6:
        content += str(m) + '. Question: ' + pattern5_list[i] + '\nExplanation: Given \"p or q\" and \"¬p\" are true, where p and q represent statements, we can infer that q is true, so the conclusion is that ' + ground_truth[i] + ' \nAnswer: ' + ground_truth[i] + '\n'
        m += 1
        i += 1
        continue
    if m == 6:
