...
while i < len(pattern4_list):
    ...
    if i % 10 == 0:
        ...
        content = ''
    if m < 6:
        content += str(m) + '. Question: ' + pattern4_list[i] + '\nExplanation: Given \"if p, then q\" and \"¬p\" are true, where p and q represent statements, we can not infer whether q is true, so there is no conclusion. \nAnswer: there is no conclusion.\n'
        m += 1
        i += 1
        continue
    if m == 6:
