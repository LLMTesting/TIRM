...
while i < len(pattern2_list):
    ...
    try:
        response = openai.ChatCompletion.create(
            model='gpt-3.5-turbo',
            messages=[
                {"role": "user", "content": pattern2_list[i]}
            ],
            temperature = 1.0
        )
    except:
        ...
        pattern2_dict['question'] = pattern2_list[i]
        pattern2_dict['response'] = result
        pattern2_results.append(pattern2_dict)
        print('the {}-th turn:'.format(i), pattern2_dict)
        time.sleep(10)
        i += 1

import json
with open("FMPR.json","w",encoding = "utf-8") as f:
    json_str = json.dumps(pattern2_results)
    f.write(json_str)
